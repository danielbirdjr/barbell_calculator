export default function calculateWeightForReps(weight, weightUnit, reps, intensityUnit, intensity, isWeightedBodyweight, bodyweight, percentageOfBodyweight, targetReps, targetIntensity) {
    ...
    // define weightForReps outside block scope
    let weightForReps;

    // get percentageForWeightForReps from mappings
    const percentageForWeightForReps = percentagesChart[targetReps][targetIntensity];

    // calculate weight for target reps 
    if (isWeightedBodyweight === false) {
        // calculate weight for reps for non-BW exercises
        weightForReps = oneRepMax * percentageForWeightForReps;
    } else {
    ...
    if (isWeightedBodyweight && weightUnit === "LB") {
        ...
        return 1.25 * Math.round(weightForReps / 1.25);
    } else if (!isWeightedBodyweight && weightUnit === "LB") {
        return 5 * Math.round(weightForReps / 5);
    } else if (!isWeightedBodyweight && weightUnit === "KG") {
        return 2.5 * Math.round(weightForReps / 2.5);
    } 
}
